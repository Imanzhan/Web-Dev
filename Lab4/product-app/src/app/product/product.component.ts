...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, ProductInfoComponent, HeaderComponent],
  template: `
    <section>
    ...
    <section class="results">
      <app-product-info
        *ngFor="let productInfo of filteredInfoList"
...
//   constructor() {
//     this.productInfoList = this.detailingService.getAllProductInfo();
...
filterResults(text: string) {
  if (!text) {
    ...
    this.filteredInfoList = this.productInfoList.filter(Productinfo =>
      Productinfo?.name.toLowerCase().includes(text.toLowerCase())
    );
  }
}
}
